 ...
 * @param scaleshift returns relative scale change of bb
 */
int fbmultitrack(IplImage *imgI, IplImage *imgJ, float *bb, float *bbnew, float *scaleshift, int num)
{
    char level = 5;
    const int numM = 10;
    const int numN = 10;
    const int nPoints = numM * numN*num;
    const int sizePointsArray = nPoints * 2;

    float* fb    = new float[nPoints];
    float* ncc   = new float [nPoints];
    char* status = new char[nPoints];
	...
	for (int j = 0; j < num; j++)
	{
		getFilledBBPoints(&bb[4*j], numM, numN, 5, tmp);
		tmp += numM*numN;
	}
	...
	for (int j = 0; j < num; j++)
	{
		...
		for(i = 0; i < numM*numN; i++)
		{
			...
			{
				startPoints[nRealPoints].x  = tpt[2 * i];
				startPoints[nRealPoints].y  = tpt[2 * i + 1];
				targetPoints[nRealPoints].x = tptTracked[2 * i];
				targetPoints[nRealPoints].y = tptTracked[2 * i + 1];
				fbLkCleaned[nRealPoints]    = tfb[i];
				nccLkCleaned[nRealPoints]   = tncc[i];
				nRealPoints++;
			}
		}
		...
		predictbb(&bb[4*j], startPoints, targetPoints, nAfterFbUsage, &bbnew[4*j], &scaleshift[j]);
